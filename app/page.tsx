...
const audience = [
  { title: 'Potential Pioneers', text: 'For leaders sensing a call to form new disciple-making communities and pioneering works.', icon: Sprout },
  { title: 'Multiplying Pastors', text: 'For pastors who want their church to become a sending church, not only a gathering church.', icon: Users },
  { title: 'Churches Ready to Send', text: 'For congregations seeking a practical pathway for discernment, development, and deployment.', icon: Compass },
];

const pathway = [
  ['01', 'Discover', 'Explore the theology, language, models, and first steps of church multiplication.'],
  ['02', 'Discern', 'Clarify calling, readiness, fit, and the next faithful step with experienced leaders.'],
  ['03', 'Develop', 'Build the character, competencies, team, and strategy needed for multiplication.'],
  ['04', 'Deploy', 'Send pioneers and teams with coaching, covering, and practical support.'],
];

const models = ['Church Launch', 'Missional Church', 'Micro Church', 'Multi-Community Church', 'The Church Collective'];
    ...
    <>
      <section className="relative overflow-hidden px-5 py-20 lg:px-8 lg:py-28">
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-14 lg:grid-cols-[1.05fr_.95fr]">
          <div>
            <p className="text-sm font-extrabold uppercase tracking-[0.34em] text-ember">A ministry pathway of Open Bible Churches</p>
            <h1 className="poster-type mt-6 text-[4.75rem] text-sand md:text-[7rem] lg:text-[8rem]">
              Mission to Multiply.
            </h1>
            <p className="mt-8 max-w-3xl text-lg leading-8 text-sand/82 md:text-xl">
            ...
            <div className="mt-10 flex flex-col gap-4 sm:flex-row">
              <Link href="/discover" className="button-primary inline-flex items-center justify-center gap-2 rounded-full px-7 py-4 text-sm font-extrabold transition">
                Start Discover: Church Multiplication 101 <ArrowRight size={18} />
              </Link>
              <Link href="/pathways" className="button-secondary inline-flex items-center justify-center rounded-full px-7 py-4 text-sm font-extrabold transition">
                Explore the Pathway
              </Link>
            </div>
          </div>
          ...
          <div className="soft-panel relative rounded-[2rem] bg-sand p-8 lg:p-10">
            <div className="mb-8 flex items-center justify-between border-b border-slate/20 pb-6">
              <LogoMark compact />
              <p className="text-right text-xs font-extrabold uppercase tracking-[0.26em] text-slate">Discover<br />Discern<br />Develop<br />Deploy</p>
            </div>
            ...
            <div className="mission-stack mt-8 text-left">
              <div className="rounded-xl bg-slate px-5 py-4 text-sand"><p className="text-xl font-black">Make Disciples</p></div>
              <div className="rounded-xl bg-ink px-5 py-4 text-sand"><p className="text-xl font-black">Develop Leaders</p></div>
              <div className="rounded-xl bg-ember px-5 py-4 text-ink"><p className="text-xl font-black">Multiply Churches</p></div>
            </div>
          </div>
        </div>
      </section>
          ...
          {audience.map((item) => {
            ...
            return (
              <div key={item.title} className="texture-card rounded-[1.7rem] p-7">
                ...
                <h3 className="mt-7 text-2xl font-black tracking-[-.025em] text-ink">{item.title}</h3>
      ...
      <section className="bg-white px-5 py-20 lg:px-8">
        ...
        <div className="brand-panel mx-auto mt-14 max-w-5xl rounded-[2rem] p-8 shadow-brand md:p-12">
          ...
          <p className="mt-6 text-xl font-semibold leading-9 text-sand/90 md:text-2xl">
          ...
          <p className="mt-6 text-lg leading-8 text-sand/78">
        ...
        <div className="mx-auto mt-14 grid max-w-7xl gap-5 md:grid-cols-4">
          {pathway.map(([number, title, text]) => (
            <div key={title} className="soft-panel rounded-[1.7rem] p-6">
              ...
              <h3 className="mt-6 text-2xl font-black tracking-[-.025em] text-ink">{title}</h3>
      ...
      <section className="bg-slate px-5 py-20 lg:px-8">
        <SectionHeader eyebrow="Models" title="Multiplication has more than one shape.">
        ...
        <div className="mx-auto mt-12 flex max-w-5xl flex-wrap justify-center gap-3">
          {models.map((model) => (
            <Link key={model} href="/models" className="rounded-full border border-sand/25 bg-sand px-5 py-3 text-sm font-bold text-slate transition hover:bg-ember hover:text-ink">
              {model}
            </Link>
          ))}
        </div>
